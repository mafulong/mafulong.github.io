...
Policy里也可以把某个resource比如ecs assume成这个role。就是policy的json，然后action是assumeRole。 `AssumeRole` 操作是用于声明一个身份可以扮演指定 IAM 角色的操作。这是一种委托身份验证机制，允许您将角色扮演的访问权限授予其他身份，而无需直接向它们分配访问权限。



比如 下面是一个示例 IAM policy，该 policy 允许具有 `myapp-deploy` 标记的 IAM 用户扮演名为 `myapp-deploy` 的 IAM 角色：
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:iam::123456789012:role/myapp-deploy",
            "Condition": {
                "StringEquals": {
                    "aws:ResourceTag/myapp": "deploy"
                }
            }
        }
    ]
...
"Assume role policy" 是 AWS Identity and Access Management (IAM) 中的一个概念，用于定义一个 IAM Role 被允许被哪些 AWS 账号或者实体 (比如 AWS 服务或者 IAM 用户) 扮演 (assume) 的权限和条件。

当一个实体想要扮演一个 IAM Role 时，需要通过调用 AWS Security Token Service (STS) 的 AssumeRole API 来获取一个临时凭证 (temporary credentials)。在调用 API 时，实体需要提供自己的身份验证信息和一个 Assume Role Policy，这个 policy 定义了扮演这个 Role 的条件和权限。如果实体满足了 policy 中的条件，就可以获得一个临时凭证，用于在一段时间内访问受 Role 保护的 AWS 资源。

Assume role policy 通常使用 JSON 格式定义，它包括了一个 principal 元素，用于指定哪些实体可以扮演这个 Role，以及一个 action 元素，用于定义这些实体可以执行的操作。除此之外，policy 还可以包含条件 (condition) 元素，用于进一步限制实体扮演这个 Role 的条件。
...
- cdk里ecs要发sns，需要给ecs授权。cdk里可以把sns topic传入ecs，然后把ecs service那个role(只有1个)，grant相应权限即可。
- 让另一个aws账号的sqs订阅当前账号的sns加权限时，可以直接给另一个aws account加权限。
- 账号A的ddb暴露给账号b的服务c，可以账号A里创建一个角色，然后创建一个Policy， grant c the role.既principle是c。

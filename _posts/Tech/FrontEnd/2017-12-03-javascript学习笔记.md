...
> - [this指向](https://juejin.cn/post/6981251280236707853)
...
5. 箭头函数，在声明的时候绑定this，而非取决于调用位置；**技巧：它的外层没有函数，this 是 window；外层有函数，看外层函数的 this 是谁，它的 this 就是谁。**
...
同时call和apply会传实参，因为会直接调用一次。而bind是返回替换this后的函数，不会执行一次。
...
比如click事件，此时假如父元素、子元素、目标元素都有click监听响应，还都是非默认的捕获阶段触发，则依次执行父元素、子元素、目标元素的响应函数。
...
> [事件委托](https://web.qianguyihao.com/04-JavaScript%E5%9F%BA%E7%A1%80/43-%E4%BA%8B%E4%BB%B6%E5%A7%94%E6%89%98.html#%E4%BA%8B%E4%BB%B6%E5%A7%94%E6%89%98)

事件委托，通俗地来讲，就是把一个元素响应事件（click、keydown......）的函数委托到另一个元素。

比如说有一个列表 ul，列表之中有大量的列表项 `<a>`标签。当我们的鼠标移到`<a>`标签上的时候，需要获取此`<a>`的相关信息并飘出悬浮窗以显示详细信息，或者当某个`<a>`被点击的时候需要触发相应的处理事件。我们通常的写法，是为每个`<a>`都绑定类似onMouseOver或者onClick之类的事件监听。

但是，上面的做法过于消耗内存和性能。**我们希望，只绑定一次事件，即可应用到多个元素上**，即使元素是后来添加的。

因此，比较好的方法就是把这个点击事件绑定到他的父层，也就是 `ul` 上，然后在执行事件函数的时候再去匹配判断目标元素。

为父节点注册 click 事件，当子节点被点击的时候，click事件会从子节点开始**向父节点冒泡**。**父节点捕获到事件**之后，开始执行方法体里

的内容：通过判断 event.target 拿到了被点击的子节点`<a>`。从而可以获取到相应的信息，并作处理。



事件委托可以减少dom操作次数，这里具体指document.getElement以及addlistener，都算dom操作。次数少了，dom就可以减少重绘重拍，进而提高性能。



用了事件委托，chrome 开发者工具就看不到这个元素的上的listener了。

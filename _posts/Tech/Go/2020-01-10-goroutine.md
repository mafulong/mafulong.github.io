...
- **示例**：执行 I/O 操作、文件读写、网络调用等。调用http等返回。Go 的 `os` 和 `io` 标准库 **使用操作系统提供的文件描述符，并结合 `netpoll` 机制**，确保文件 I/O 只阻塞当前 Goroutine，而不会阻塞整个 OS 线程。netpoll唤醒goroutine
- 线程行为
...
- 直接使用 `syscall` 进行文件 I/O（如 `syscall.Read()`、`syscall.Write()`），那么 OS 线程可能会被阻塞，

- 线程行为

  - 如果 Goroutine 调用了一个会导致操作系统层面阻塞的系统调，那么该系统调用会阻塞整个线程。这是操作系统的规则，系统调用必须阻塞线程。
...
之前都是协作式调度，主动让出cpu。 时间片轮转。



在Go语言的`v1.2`版本就实现了基于协作的抢占式调用，这种调用的基本原理就是：

1. 当`sysmon`监控线程发现有协程的执行时间太长了，那么会友好地为这个协程设置抢占标记；
2. 当这个协程`调用（call）`一个函数时，会检查是否扩容栈，而这里就会检查抢占标记，如果被标记，则会让出CPU，从而实现调度。

但是这种调度方式是协程主动的，是`基于协作`的，**在函数调用时进行抢占检查**。 但是他无法面对一些场景，比如在死循环中没有任何调用发生，那么这个协程将永远执行下去，永远不会发生调度，这显然是不可接受的。

于是，在`v1.14`版本，Go终于引入了`基于信号`的抢占式调度。 抢占式调度则能避免由于死循环导致的任意时间的垃圾回收延迟。



基于信号的抢占式调度抢占会在下列时机发生：

...
**陷阱:** **声明defer时，defer函数的实参就会赋值，注意赋值的是不是引用，如果是引用可能会变化延迟。**
...
如果是A的指针类型了实现了接口B，那A的非指针对象是不能赋值给非指针对象B的，只能A的指针对象赋值给非指针对象B。看是否实现时是区分是指针版本实现的还是非指针实现的，只有对应实现的才能子类转父类。如下。7

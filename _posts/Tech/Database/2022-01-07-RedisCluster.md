...
失效检测 -> 整个主节点选举的过程可分为**申请、授权、升级、同步**四个阶段



- **主节点失效检测**
  - 集群节点会定期 **PING-PONG** 交互，检测彼此状态：
    - 超过 `node timeout` 未响应，则标记为 **PFAIL（可能失败）**。
    - 当大多数主节点（n/2+1）都认为该节点失效，则标记为 **FAIL**，并广播给整个集群。
- **从节点选举新的主节点**
  - 从节点会判断是否符合选举资格：
    - 必须是失效主节点的从节点。
    - 该主节点管理的哈希槽不为空。
    - 复制连接断线时间未超限。
  - **偏差小的从节点优先申请选举**，向其他主节点发送授权请求。
- **主节点授权**
  - 其他主节点会检查：
    - 申请者是故障主节点的从节点。
    - `currentEpoch` 较新，并且 `configEpoch` 合理。
    - 申请者自身状态正常（无 FAIL/PFAIL）。
  - 如果通过，主节点向申请者发送 **CLUSTERMSG_TYPE_FAILOVER_AUTH_ACK** 授权。
- **新主节点升级**
  - 获得多数授权后，从节点：
    - **接管原主节点的哈希槽**。
    - 广播 `PONG` 消息，宣告自己成为 **新的主节点**。
    - 其他从节点更新配置，并同步新主节点数据。
- **原主节点恢复**
  - 如果原主节点恢复：
    - 它的 `configEpoch` 低于新的主节点，会自动变为其 **从节点**。
    - 如果它恢复 **早于 Failover 完成**，则不会发生主从切换。

---

### 手动 Failover（Manual Failover）

- **运维功能**，允许手动将从节点提升为主节点，即使主节点仍存活。
- 主要区别：
  - **触发方式**：手动执行 `CLUSTER FAILOVER` 命令，仅作用于从节点。
  - **申请条件**：
    - 无需主节点失效。
    - 固定 5 秒超时时间，仅命令目标从节点会发起申请。
- **手动 Failover 方式**：
  - **普通模式**（默认）：等待数据完全同步后进行切换，期间主节点暂停写操作。
  - **Force 模式**：不等待数据同步，直接强制切换。

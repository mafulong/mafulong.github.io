...
```python
# 递归插入
def insertTreeNode(root, val) -> TreeNode:
    if root is None:
        root = TreeNode()
        root.val = val
        return root
...
def deleteTreeNode(root: TreeNode, val):
    if root is None:
        return
    if root.val == val:
        if root.left is None and root.right is None:
            ...
            root.val, replaceNode.val = replaceNode.val, root.val

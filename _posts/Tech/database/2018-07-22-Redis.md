...
Redis 是通过 C 语言实现的，但 Redis 并没有直接使用 C 语言的字符串，而是自己实现了一种叫做动态字符串 SDS 的类型。类似java的一个arrayList。 是可修改的字符串。额外存了长度。
...
Redis 支持同时启用 RDB 和 AOF。**优先级**：如果两者都启用，Redis 在重启时会优先加载 AOF 文件。

在 Redis 4.0 后，增加了 AOF 和 RDB 混合的数据持久化机制： 把数据以 RDB 的方式写入文件，再将后续的操作命令以 AOF 的格式存入文件，既保证了 Redis 重启速度，又降低数据丢失风险。如果把混合持久化打开，AOF 重写的时候就直接把 RDB 的内容写到 AOF 文件开头。这样做的好处是可以结合 RDB 和 AOF 的优点, 快速加载同时避免丢失过多的数据。当然缺点也是有的， AOF 里面的 RDB 部分是压缩格式不再是 AOF 格式，可读性较差。
...
- 正常关闭 (`SHUTDOWN`) 并重启
  ...
  - Redis 会将内存中的数据写入持久化文件（如果持久化开启），重启后可以恢复。
- 非正常关闭（如强制杀进程或断电）
  
  - 如果没有及时保存快照或同步 AOF，数据可能丢失。可能丢失最近的部分写入。
...
AOF 采用的是**写后日志**的方式，Redis 先执行命令把数据写入内存，然后再记录日志到文件中。AOF 日志记录的是操作命令，不是实际的数据，如果采用 AOF 方法做故障恢复时需要将全量日志都执行一遍。
...
Redis 中的 “0 号和 1 号哈希表” 是 Redis 底层 `dict` 结构中的双哈希表，用于管理键值对和支持渐进式 rehash。它们与 Redis 的哈希数据类型（`HSET`、`HGET` 等）无关，而是 Redis 存储引擎的实现细节，用于优化性能和动态扩容。



**哈希表 0 和哈希表 1 的用途**：

- **哈希表 0**：当前正在使用的主哈希表，存储了所有的键值对。
- **哈希表 1**：在执行哈希表扩容或收缩时（如重新哈希操作），用作临时存储。
...
- 文件事件（连接相关，相应请求）-> 时间事件（定时任务，以无序列表保存）
...
twemproxy：既要处理一个client发来的多个命令，又要将到同一个下游redis server的命令缓存起来一起发



Twemproxy（也称为 Nutcracker）是由 Twitter 开发的一款轻量级的中间件，主要用于：

- 代理多个 Redis 或 Memcached 实例。
- 提供简单的分布式支持，而无需客户端直接实现分布式逻辑。
- 支持将请求按照特定的分片算法分发到多个 Redis 或 Memcached 实例。
- 客户端只需连接 Twemproxy，而无需直接管理多个实例。
- 客户端与 Twemproxy 之间是长连接，减少了频繁建立连接的开销。
- 支持一致性hash分片
...
- 使用 `pipeline`
  ...
  - 如果需要批量执行多种 Redis 命令（如 `SET` 和 `GET` 的混合操作），或者希望减少网络通信延迟。

- 使用 `mget`

  - 如果场景仅涉及批量 `GET` 操作，`mget` 是更高效的选择，因为它直接对多键查询进行了底层优化。

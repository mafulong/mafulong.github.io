...
相比读者优先，增加writeCount和writeCountSignal在写过程里保证写优先，第一次写时申请read信号量，之所以用read是因为不能多次写，所以read不同于fileSrc
...
semaphore read = 1; //读写都竞争，每次读需要它，第一次写需要它
...
相比读者优先，就是写和读外面有个竞争keySignal,然后写最外面再来个竞争OneSignal.

```c++
/* 读者队列初始值为0，其他资源初始值为1*/
int readCount = 0;
semaphore readCountSignal = 1; //保护readCount
semaphore keySignal = 1; //读写每次都用，保证公平
